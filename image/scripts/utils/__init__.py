    ...
    logging.config.dictConfig(
        {
            ...
            "handlers": {"default": {"level": "INFO", "formatter": "colored", "class": "logging.StreamHandler"}},
            "loggers": {
                "": {"handlers": ["default"], "level": "INFO", "propagate": True},
                "django.request": {"handlers": ["default"], "level": "WARN", "propagate": False},
            },
        }
    )
...
def get_match_pids(pattern):
    pgrep_output = subprocess.check_output('pgrep -f "{}" || true'.format(pattern), shell=True).strip()
...
def git_commit_time(refspec):
    return int(get_command_output('git log -1 --format="%ct" {}'.format(refspec)).strip())
...
def _add_default_context(context):
    default_context = {"current_timestr": datetime.datetime.now().strftime("%m/%d/%Y %H:%M:%S")}
